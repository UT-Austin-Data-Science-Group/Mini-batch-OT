...
def main():
    ...
    train_loader = torch.utils.data.DataLoader(
        datasets.MNIST(args.datadir, train=True, download=True, transform=transforms.Compose([transforms.ToTensor()])),
        batch_size=args.k * args.m,
        shuffle=True,
    ...
    for epoch in range(0, args.epochs):
        ...
        if (epoch % args.fid_each == 0) or (epoch == args.epochs - 1):
            ...
            for _, (input, y) in enumerate(test_loader, start=0):
                ...
                W = compute_true_Wasserstein(
                    data.view(data.shape[0], -1).to("cpu"), fake.view(data.shape[0], -1).to("cpu")
                )
                break
            model.train()
