    ...
    def train_minibatch(
        ...
        if method == "sliced":
            ...
            for i in range(k):
                ...
                errD_real = 1.0 / (k**2) * criterion(y_data, label)
                errD_real.backward()
            optimizer.step()
            optimizer.zero_grad()
            for j in range(k):
                ...
                criterion = nn.BCELoss(reduction="sum")
                errD_real = 1.0 / (k**2) * criterion(y_fake, label)
                errD_real.backward()
            optimizer.step()
        else:
            optimizer.zero_grad()
            for i in range(k):
                for j in range(k):
                    ...
                    if bomb or ebomb:
                        ...
                        mloss = -1.0 / (k**2) * torch.sum(pi * cost_matrix)
                    Dloss += mloss
                    # Estimate gradient
                    mloss.backward()
            # Gradient descent
                        ...
                        if method == "sliced":
                            gloss.append(
                                sliced_wasserstein_distance(
                                    feature_data_mb, feature_fake_mb, num_projections=L, device=self.device
                                )
                            )
                        else:
                            cost_matrix = torch.cdist(feature_data_mb, feature_fake_mb) ** 2
