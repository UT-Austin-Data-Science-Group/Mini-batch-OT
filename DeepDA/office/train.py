...
import lr_schedule
from data_list import ImageList, ImageList_label, BalancedBatchSampler
import random
import ot
...
def image_label(loader, model, threshold=0.9, out_dir=None):
    ...
    with torch.no_grad():
        with open(out_path, 'w') as f:
            for i in tqdm(range(len(loader['target_label']))):
...
def train(config):
    ...
    if config['args'].stratify_source:
        ...
        dset_loaders["source"] = DataLoader(dsets["source"], batch_sampler=source_sampler, num_workers=config['args'].num_worker)
    else:
