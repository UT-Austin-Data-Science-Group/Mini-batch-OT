...
def train(args):
    ...
    dset_loaders["source"] = DataLoader(
        dsets["source"], batch_size=train_bs, shuffle=True, num_workers=args.worker, drop_last=True
    )
    dset_loaders["target"] = DataLoader(
        dsets["target"], batch_size=train_bs, shuffle=True, num_workers=args.worker, drop_last=True
    )
    ...
    for i in tqdm(range(args.max_iterations + 1)):
        ...
        if (i % args.test_interval == 0 and i > 0) or (i == args.max_iterations):
            ...
            if mean_ent < best_ent:
                best_ent, best_acc, best_iter = mean_ent, temp_acc, i
        ...
        if i % args.test_interval == 0:
            ...
            if not len_share == 0:
                dset_loaders["middle"] = DataLoader(
                    dsets["source"], batch_size=len_share, shuffle=True, num_workers=args.worker, drop_last=True
                )
        ...
        transfer_loss = my_loss.DANN(
            features, ad_net, entropy, network.calc_coeff(i, 1, 0, 10, args.max_iterations), cls_weight, len_share
        )
    ...
    args.out_file.flush()
    print(log_str)
    with open("result_BA3US.txt", "a") as f:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--dset", type=str, default="office_home", choices=["office", "office_home", "imagenet_caltech"]
    )
